...
ST_INLN Sym *struct_find(int v)
{
    if (TOK_IDENT <= v && v < tok_ident)
        return table_ident[v - TOK_IDENT]->sym_struct;
    return NULL;
}
...
static void apply_visibility(Sym *sym, CType *type)
{
    ...
    vis = sym->type.t & VT_VIS_MASK;
    vis2 = type->t & VT_VIS_MASK;
...
/* Store vtop to a register belonging to class 'rc'. lvalues are
   converted to values. Cannot be used if vtop cannot be converted to
...
ST_FUNC int gv(int rc)
{
    ...
    if (vtop->type.t & VT_BITFIELD) {
        ...
        if (is_float(vtop->type.t) &&
            ...
            offset = roundup(data_section->data_offset, align);
...
/* rotate first n stack elements to the bottom
...
/* rotate first n stack elements to the top
   I1 ... In -> In I1 ... I(n-1)  [top is right] */
...
static void gen_opl(int op)
{
    int t, a, b, op1, c, i;
    int func;
    unsigned short reg_iret, reg_lret;
    SValue tmp;
    ...
    if (c1 && c2) {
        ...
        if (c2 &&
            (((op == '*' || op == '/' || op == TOK_UDIV || op == TOK_PDIV) &&
              l2 == 1) ||
             ((op == '+' || op == '-' || op == '|' || op == '^' ||
               op == TOK_SHL || op == TOK_SHR || op == TOK_SAR) && l2 == 0) ||
             (op == '&' && l2 == -1))) {
            /* nothing to do */
            vtop--;
                  ...
                  int case_reg, int is_expr)
{
    ...
    if (tok == TOK_IF) {
        ...
        if (vla_sp_loc !=
            (saved_vla_sp_loc ==
             &vla_sp_root_loc ? &vla_sp_root_loc : &block_vla_sp_loc)) {
            vla_sp_loc = saved_vla_sp_loc;
            gen_vla_sp_restore(*vla_sp_loc);
        }
        vla_flags = (vla_flags & ~VLA_SCOPE_FLAGS)
            | (saved_vla_flags & VLA_SCOPE_FLAGS);
    ...
    if (type->t & VT_VLA) {
        ...
        if ((tok == TOK_LSTR &&
            ...
            while (tok == TOK_STR || tok == TOK_LSTR) {
                ...
                cstr_len = tok == TOK_STR ? cstr->size :
                    cstr->size / sizeof(nwchar_t);
                cstr_len--;
                nb = cstr_len;
                ...
                if (!size_only) {
                    ...
                    if (sec && tok == TOK_STR && size1 == 1) {
                        ...
                        for(i = 0; i < nb; i++) {
                            ch = tok == TOK_STR ? ((unsigned char *)cstr->data)[i] :
...
static void put_func_debug(Sym *sym)
{
    ...
    /* gdb wants a line at the function -- gr */
...
static int decl0(int l, int is_for_loop_init)
{
    ...
    for (;;) {
        ...
        for (;;) { /* iterate thru each declaration */
